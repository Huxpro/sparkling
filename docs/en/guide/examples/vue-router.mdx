...
browser**. Tap **push('/users')** and a second container stacks on top (the
header's `depth` becomes 1); the **browser back button** pops it, because the
shell owns the whole tab and drives real browser history.
